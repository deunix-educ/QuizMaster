...
class MqttBase(object):
    def __init__(self, **p):
        ...
        try:
            ...
            self.ca_cert        = p.get('ca_cert', None)
            ...
            self.on_receive_message_callback = p.get('on_receive_message', self._on_receive_message_callback)
            ...
            logger.error(f'MqttBase error {e}')
    ...
    def _on_receive_message_callback(self, topic, payload):
        return None

    def _on_message(self, client, userdata, message):
        try:
            if self.on_receive_message_callback(message.topic, message.payload):
                return
            try:
